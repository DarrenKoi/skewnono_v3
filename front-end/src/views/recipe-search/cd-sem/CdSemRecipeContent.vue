<template>
  <div>
    ...
    <div class="flex items-center justify-between mb-6">
      ...
      <Button label="장비 선택으로 돌아가기" icon="pi pi-arrow-left" @click="goBack" severity="secondary" size="small" outlined />
    </div>
    ...
    <div class="space-y-6">
      ...
      <div class="bg-surface-0 dark:bg-surface-900 rounded-xl p-6 shadow-sm border">
        ...
        <div class="flex flex-col sm:flex-row gap-4">
          <AutoComplete v-model="recipeOpenRecipe" :suggestions="filteredRecipesOpen" :forceSelection="true"
            @complete="searchRecipeOpen" placeholder="Recipe 이름을 입력하세요..." class="flex-1" :dropdown="true"
            :minLength="1" />
          <Button label="Recipe 열기" icon="pi pi-external-link" @click="openRecipe" :disabled="!recipeOpenRecipe"
            class="w-full sm:w-auto" />
        </div>

        <!-- Show Recipe Open Content -->
        <div v-if="showRecipeOpen && recipeOpenRecipe" class="mt-6 pt-6 border-t">
          <RecipeOpenView :recipe-name="recipeOpenRecipe" />
        </div>
      </div>
      ...
      <div class="bg-surface-0 dark:bg-surface-900 rounded-xl p-6 shadow-sm border">
        ...
        <div class="flex flex-col sm:flex-row gap-4">
          <AutoComplete v-model="horizontalCheckRecipe" :suggestions="filteredRecipesHorizontal" :forceSelection="true"
            @complete="searchRecipeHorizontal" placeholder="횡전개 체크할 Recipe를 입력하세요..." class="flex-1" :dropdown="true"
            :minLength="1" />
          <Button label="횡전개 체크 실행" icon="pi pi-check-square" @click="checkHorizontal"
            :disabled="!horizontalCheckRecipe" class="w-full sm:w-auto" />
        </div>

        <!-- Show Horizontal Check Content -->
        <div v-if="showHorizontalCheck && horizontalCheckRecipe" class="mt-6 pt-6 border-t">
          <RecipeHorizontalCheckView :recipe-name="horizontalCheckRecipe" />
        </div>
      </div>
      ...
      <div class="bg-surface-0 dark:bg-surface-900 rounded-xl p-6 shadow-sm border">
        ...
        <div class="space-y-4">
          <div class="flex flex-col sm:flex-row gap-4">
            <AutoComplete v-model="measurementHistoryRecipe" :suggestions="filteredRecipesMeasurement"
              :forceSelection="true" @complete="searchRecipeMeasurement" placeholder="측정 기록을 조회할 Recipe를 입력하세요..."
              class="flex-1" :dropdown="true" :minLength="1" />
          </div>
          ...
          <div v-if="measurementHistoryRecipe" class="flex flex-col sm:flex-row gap-4">
            <Calendar v-model="dateRange" selectionMode="range" :manualInput="false" dateFormat="yy/mm/dd"
              placeholder="날짜 범위 선택" showIcon class="flex-1" />
            <Button label="측정 기록 조회" icon="pi pi-history" @click="checkMeasurementHistory"
              :disabled="!measurementHistoryRecipe || !dateRange || dateRange.length !== 2" class="w-full sm:w-auto" />
          </div>
        </div>
        ...
        <div v-if="showMeasurementHistory && measurementHistoryRecipe && dateRange" class="mt-6 pt-6 border-t">
          <RecipeMeasurementHistoryView :recipe-name="measurementHistoryRecipe" :date-range="dateRange" />
        </div>
      </div>
    </div>
  </div>
...
import RecipeOpenView from './RecipeOpenView.vue'
import RecipeHorizontalCheckView from './RecipeHorizontalCheckView.vue'
import RecipeMeasurementHistoryView from './RecipeMeasurementHistoryView.vue'
