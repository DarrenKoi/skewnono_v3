    ...
    <div class="flex flex-col items-start gap-4 mb-8">
      ...
      <div class="flex flex-col gap-2">
        ...
        <div class="text-surface-500 dark:text-surface-300 text-lg">
          CD-SEM & HV-SEM 장비의 스토리지 사용량 및 레시피 수 정보
      ...
      <div class="flex flex-col gap-4">
        <div class="flex justify-between items-center">
          <div class="flex flex-col gap-1">
            ...
            <p class="text-sm text-surface-500">스토리지 사용률 높은 순으로 정렬 (Update 주기 : 24 Hrs)</p>
          </div>
        </div>
        ...
        <div class="mb-4">
          ...
          <div class="flex flex-wrap gap-2">
            <Button label="전체" :severity="selectedEquipmentType === null ? 'primary' : 'secondary'"
              :outlined="selectedEquipmentType !== null" size="small" @click="selectEquipmentType(null)" />
            <Button v-for="category in equipmentCategories" :key="category.name"
              :label="`${category.name} (${category.count})`"
              :severity="selectedEquipmentType === category.name ? 'primary' : 'secondary'"
              :outlined="selectedEquipmentType !== category.name" size="small"
        ...
        <DataTable v-else :value="storageData" stripedRows paginator :rows="10" :rowsPerPageOptions="[10, 20, 50]"
          sortMode="multiple" removableSort :globalFilterFields="['eqp_id', 'fab_name', 'fac_id', 'eqp_model_cd']"
          ...
          <template #header>
            <div class="flex justify-between items-center">
              ...
              <span class="p-input-icon-left">
                ...
                <InputText v-model="globalFilterValue" placeholder="전체 검색" class="w-64" />
              </span>
            </div>
          </template>
          ...
          <Column field="fab_name" header="FAB NAME" sortable></Column>

          <Column header="카테고리" sortable>
            <template #body="{ data }">
              <Tag :value="getModelCategory(data.eqp_model_cd)"
          ...
          <Column field="percent" header="사용률" sortable>
            <template #body="{ data }">
              <div class="flex items-center gap-2">
                <ProgressBar :value="parseInt(data.percent)" :showValue="false" class="w-full" :class="{
